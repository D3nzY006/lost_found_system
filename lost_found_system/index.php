        ...
        if ($action === 'claim_review') {
            ...
            try {
                ...
                if ($decision === 'Approved') {
                    $s = $pdo->prepare("UPDATE found_item_records SET status='Claimed' WHERE found_id=? AND status='Unclaimed'"); $s->execute([$cl['found_id']]);
                    if ($s->rowCount() !== 1) throw new RuntimeException('The found item is already claimed.');
    ...
    <header class="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur">
      <div class="flex items-center justify-between gap-3 px-4 py-3 lg:px-8">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
          <button class="relative grid h-10 w-10 place-items-center rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50" aria-label="Notifications">
            ...
            <?php $unread = 0; try { $unread = (int)db()->query("SELECT COUNT(*) FROM claim_records WHERE status='Pending'")->fetchColumn(); } catch (Throwable $e) {} ?>
            <?php if ($unread > 0): ?><span class="absolute -right-1 -top-1 grid h-5 min-w-[1.25rem] place-items-center rounded-full bg-rose-500 px-1 text-[10px] font-bold text-white"><?=$unread?></span><?php endif; ?>
          </button>
          <div class="relative">
...
<footer class="border-t border-slate-200 bg-white py-6 text-center text-xs text-slate-500">© <?=date('Y')?> Lost &amp; Found Management System · PHP + MySQL</footer>
<script>
(function(){
  ...
  if(pb && pm){ pb.addEventListener('click',(e)=>{e.stopPropagation(); pm.classList.toggle('hidden');}); document.addEventListener('click',()=>pm.classList.add('hidden')); }
  ...
  <div class="grid w-full max-w-5xl gap-6 lg:grid-cols-2">
    ...
    <div class="card p-8">
      ...
      <form method="post" class="mt-6 space-y-4">
        ...
        <div><label class="block text-xs font-bold uppercase tracking-wide text-slate-600">Password</label><input type="password" name="password" required class="input mt-1" placeholder="demo123"></div>
        <button class="btn btn-primary w-full justify-center">Sign in</button>
        <p class="text-xs text-slate-500">Demo: <b>U001–U010</b> · password <b>demo123</b> · admin <b>U004</b></p>
      </form>
...
<?php elseif ($page === 'lost'):
  ...
  $params = [];
  if ($q !== '') { $sql .= ' AND (l.item_name LIKE ? OR c.category_name LIKE ? OR l.color LIKE ? OR l.location_lost LIKE ? OR l.lost_id LIKE ?)'; $like="%$q%"; $params=[$like,$like,$like,$like,$like]; }
...
<div class="card p-5">
  ...
  <form class="mt-4 grid gap-3 sm:grid-cols-[1fr_180px_auto]">
    <input name="q" value="<?=e($q)?>" placeholder="Search by item, category, color, location, or ID" class="input">
...
<div class="mt-4 card p-0 overflow-hidden">
  ...
  <table class="table w-full min-w-[1100px] text-left">
    <thead class="bg-slate-50"><tr><th class="px-4 py-3">Record ID</th><th class="px-4 py-3">Item Name</th><th class="px-4 py-3">Category</th><th class="px-4 py-3">Description</th><th class="px-4 py-3">Date Lost</th><th class="px-4 py-3">Time</th><th class="px-4 py-3">Location</th><th class="px-4 py-3">Reported By</th><th class="px-4 py-3">Status</th><th class="px-4 py-3 text-right">Actions</th></tr></thead>
    <tbody class="divide-y divide-slate-100">
      ...
      <?php endforeach; if (!$rows): ?><tr><td colspan="10" class="px-4 py-12 text-center text-slate-500">No lost records match your search.</td></tr><?php endif; ?>
    </tbody>
  </table>
  </div>
</div>
<?php elseif ($page === 'found'):
  ...
  $params = [];
  if ($q !== '') { $sql .= ' AND (f.item_name LIKE ? OR c.category_name LIKE ? OR f.color LIKE ? OR f.location_found LIKE ? OR f.found_id LIKE ?)'; $like="%$q%"; $params=[$like,$like,$like,$like,$like]; }
...
<div class="mt-4 card p-0 overflow-hidden">
  <div class="overflow-x-auto scrollbar-thin">
  <table class="table w-full min-w-[1000px] text-left">
    <thead class="bg-slate-50"><tr><th class="px-4 py-3">Item ID</th><th class="px-4 py-3">Item Name</th><th class="px-4 py-3">Category</th><th class="px-4 py-3">Description</th><th class="px-4 py-3">Date Found</th><th class="px-4 py-3">Location</th><th class="px-4 py-3">Found By</th><th class="px-4 py-3">Status</th><th class="px-4 py-3 text-right">Actions</th></tr></thead>
    <tbody class="divide-y divide-slate-100">
      <?php foreach ($rows as $r): ?>
      <tr class="hover:bg-slate-50">
      ...
      <?php endforeach; if (!$rows): ?><tr><td colspan="9" class="px-4 py-12 text-center text-slate-500">No found items match your search.</td></tr><?php endif; ?>
    </tbody>
  </table>
  </div>
</div>
<?php elseif ($page === 'claims'):
  require_login();
  $filter = $_GET['status'] ?? '';
  $sql = "SELECT c.*,f.item_name,f.color,f.location_found,f.status AS found_status,u.name AS claimant,v.name AS verifier FROM claim_records c JOIN found_item_records f ON f.found_id=c.found_id JOIN users u ON u.user_id=c.user_id LEFT JOIN users v ON v.user_id=c.verified_by";
  $params = [];
  if (!is_admin()) { $sql .= ' WHERE c.user_id=?'; $params[] = user()['user_id']; }
  if (in_array($filter, ['Pending','Approved','Rejected'], true)) { $sql .= (strpos($sql,'WHERE')?' AND ':' WHERE ').'c.status=?'; $params[] = $filter; }
...
<div class="card p-5">
  <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    ...
    <form class="flex gap-2"><input type="hidden" name="page" value="claims"><select name="status" class="input" onchange="this.form.submit()"><option value="">All</option><option <?=$filter==='Pending'?'selected':''?>>Pending</option><option <?=$filter==='Approved'?'selected':''?>>Approved</option><option <?=$filter==='Rejected'?'selected':''?>>Rejected</option></select></form>
  </div>
</div>
<div class="mt-4 card p-0 overflow-hidden">
  <div class="overflow-x-auto scrollbar-thin">
  <table class="table w-full min-w-[1000px] text-left">
    ...
    <tbody class="divide-y divide-slate-100">
      ...
      <?php endforeach; if (!$rows): ?><tr><td colspan="7" class="px-4 py-12 text-center text-slate-500">No claim records found.</td></tr><?php endif; ?>
    </tbody>
  </table>
  </div>
</div>
...
<div class="card p-5">
  <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    ...
    <form class="flex gap-2"><input name="q" value="<?=e($q)?>" placeholder="Search by ID, name, role, department" class="input"><button class="btn btn-primary">Search</button></form>
  </div>
</div>
<div class="mt-4 card p-0 overflow-hidden"><div class="overflow-x-auto scrollbar-thin">
  <table class="table w-full min-w-[900px] text-left">
    ...
    <tbody class="divide-y divide-slate-100">
      <?php foreach ($rows as $u): ?>
        <tr class="hover:bg-slate-50">
          ...
          <td class="px-4 py-3"><div class="flex items-center gap-2"><span class="grid h-8 w-8 place-items-center rounded-full bg-blue text-xs font-bold text-white"><?=e(initials($u['name']))?></span><span class="font-semibold text-navy"><?=e($u['name'])?></span></span></div></td>
